...
function trainer_J_fixedV(plmvar, n_epochs, V; 
    ...
    m = (K = T.(init_fun(N, N, H)), h = T.(init_fun(q, N)))
    ...
    for i in 1:n_epochs
        ...
        for (z,w) in loader
            _w = w/sum(w)
            g = gradient(x->foo(x.K, V, x.h, z, _w; lambda = T(λ)), m)[1]
            update!(t,m,g)
            if ar == true
                @tullio m.K[i,j,h] = m.K[i,j,h] * (j<i)
            end
        ...
        if i % 10 == 0
            ...
            l[i], lr[i] = foos(m.K, V, m.h, plmvar.Z, _w; lambda = T(λ))
            p[i] = round((PPV[N]),digits=3)
            p2[i] = round((PPV[2*N]),digits=3)
    ...
    @time K_mf, V_mf = HopfieldDCA.minimize_opt_loss_ij(Int.(plmvar.Z),
        V, N, H, alpha = alpha);
    
    m_anal = (K = K_mf, V = V_mf); 
    
    #@time m_anal = 0; #optimize_K(plmvar.Z, V, N, H; q = q, optimizer=LBFGS()) 
    ...
    if par_save
        s = HopfieldDCA.score(m.K, V); PPV_h = HopfieldDCA.compute_PPV(s, structfile);
        s_anal = HopfieldDCA.score(m_anal.K, m_anal.V); PPV_h_anal = HopfieldDCA.compute_PPV(s_anal, structfile);
        @load joinpath(folder, "PPV_s.jld2") ss
        @load joinpath(folder, "PPV_plm.jld2") splm
        
        @save joinpath(folder, "fixVort$(orthog)_parsH$(H)η$(η)λ$(λ)T$(n_epochs).jld2") m PPV_h m_anal PPV_h_anal
                
        fig,ax1 = plt.subplots()         
        ax1.plot(ss, label="Stucture")
        ax1.plot(splm, label="PLM")
        ax1.plot(PPV_h, label="Hop")
        ax1.plot(PPV_h_anal, label = "mfHop")
        ax1.set_xscale("log")
        ax1.set_xlabel("Number of Predictions")
        ax1.set_ylabel("PPV")
        ax1.legend()
