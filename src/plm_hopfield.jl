...
1. `Ktensor`: the 3 dimensional Array (H×L×L)  of the couplings  K[a,b,i,j]
2. `Htensor`: the 2 dimensional Array (Hxq) of the fields in zero-sum gauge h[a,i]
...
* `lambdaK::Real=0.01` coupling L₂ regularization parameter (lagrange multiplier)
* `lambdaV::Real=0.01` field L₂ regularization parameter (lagrange multiplier)
...
# Example
```
julia> res = hop_plmDCA(Z,W,lambdaJ=0.01,lambdaH=0.01,epsconv=1e-12);
```
"""
function hop_plmdca_asym(Z::Array{T,2}, W::Vector{Float64}, H::Int;
    ...
    all(x -> x > 0, W) || throw(DomainError("vector W should be normalized and with all positive elements"))
    isapprox(sum(W), 1) || throw(DomainError("sum(W) ≠ 1. Consider normalizing the vector W"))
    N, M = size(Z)
    M = length(W)
    q = Int(maximum(Z))
    plmalg = HopPlmAlg(method, verbose, epsconv, maxit)
    plmvar = HopPlmVar(N, M, q, H, lambdaK, lambdaV, Z, W)
    Jmat, pslike = minimize_pl_asym(plmalg, plmvar)
    score, _, Ktensor, Vtensor = compute_score(Jmat, plmvar, min_separation)
    return HopPlmOut(pslike, Ktensor, Vtensor, score)
end

hop_plmdca(Z, W; kwds...) = hop_plmdca_asym(Z, W, H; kwds...)
...
```
"""
function hop_plmdca_asym(filename::String, H::Int;
    ...
    hop_plmdca_asym(Z, W, H; kwds...)
end
"""
    plmdca(filename::String; kwds...) -> PlmOut
Run [`plmdca_asym`](@ref) on the fasta alignment in `filename`
It returns a struct `::HopPlmOut` containing four fields:
...
```
"""
hop_plmdca(filename::String, H; kwds...) = plmdca_asym(filename, H; kwds...)



function minimize_pl_asym(alg::HopPlmAlg, var::HopPlmVar)
...
function hop_pl_site_grad!(x::Vector{Float64}, grad::Vector{Float64}, site::Int, plmvar::HopPlmVar)
    LL = length(x)
    q2 = plmvar.q2
    q = plmvar.q
    N = plmvar.N
...
function fillvecene!(vecene::Vector{Float64}, x::Vector{Float64}, site::Int, 
    IdxSeq::AbstractArray{Int,1}, q::Int, N::Int)
    q2 = q * q
...
function l2norm_asym(vec::Array{Float64,1}, plmvar::HopPlmVar)
    q = plmvar.q
    N = plmvar.N
...
function compute_score(Jmat::Array{Float64,2}, var::HopPlmVar, min_separation::Int)
    q = var.q
    N = var.N
